...
class Particle:  # a class, creates a new type of object where common features are stored.
    ...
    def motion(self):
        ...
        self.y - + math.cos(self.angle) * self.speed  # y is negative since the y-axis on comp graphics is inverted
...
while running:
    ...
    for particle in my_particles:  # moved from earlier in the code so that simulation
        # will run for as long as the code is running/screen is on.
        particle.display()
        particle.motion()  # NEW, to account for motion f(x)
        pygame.display.flip()
